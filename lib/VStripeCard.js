...
import merge from 'deepmerge'; // Styles

import "..\\src\\VStripeCard.sass"; // Extensions and Components
// @ts-ignore

import { VProgressLinear, VTextField } from 'vuetify/lib'; // Create Base Mixins and Define Custom Properties
    ...
    isDark(val, oldVal) {
      ...
      if (val !== oldVal && this.card !== null) {
        ...
        const {
          ...
          theme,
          customStyle
        } = this; // generate styles to match the theme

        const style = this.genStyle(customStyle, fontName, isDark, theme); // then update the card
  ...
  methods: {
    ...
    genCard() {
      ...
      const {
        ...
        theme,
        zip
      } = this; // use them to generate the options we'll need to generate the Stripe and card instances

      const style = this.genStyle(customStyle, fontName, isDark, theme);
      const cardProps = this.genCardProps({
        disabled,
        hideIcon,
        hidePostalCode,
        iconStyle,
     ...
     */
    genFont(font = 'Roboto') {
      const cssSrc = this.isURL(font) ? font : `https://fonts.googleapis.com/css?family=${encodeURI(font)}:400`;
      return {
        fonts: [{
          cssSrc
        }]
     ...
     * @param   {string} font
     */
    genStyle: (customStyle, fontName, isDark, theme) => {
      const defaults = {
        base: {
    ...
    loadStripe() {
      ...
      if (typeof Stripe === 'function') {
        ...
        const src = 'https://js.stripe.com/v3/'; // Is it already being loaded by another component?
        ...
        if (document.querySelector(`script[src="${src}"]`)) {
          ...
          if (typeof this.$loadScript === 'undefined') {
            ...
            this.$loadScript(src).then(() => {
              ...
              this.$root.$emit('stripe-loaded'); // and generate the card
      ...
      if (this.stripe && this.card) {
        if (this.create === 'token') {
          const {
            token,
            error
          } = await this.stripe.createToken(this.card, this.options);
          ...
          const {
            source,
            error
          } = await this.stripe.createSource(this.card, this.options);
